...
#Cool colorpalette for awesome plots
library("wesanderson")
names(wes_palettes)
pal <- wes_palette("GrandBudapest2", 4, type = "continuous")
```
...
brm_out <- brm(
  MeanES | se(SdES) ~ 1 + (1 | StudyRef), # ~ 1= Intercept (We want the mean effectsize), (1 | study) -> Reandom intercepts for study, yi = means, se(sei) = Std Err of the means (So you don't have to calculate it)
  prior = set_prior("uniform(-10, 1000)", class = "sd"), #Uniform prior
  data = meta, 
  cores = 4
)
summary(brm_out)
